...
class DockerHandle implements SessionHandle {
  ...
  async start(): Promise<void> {
    ...
    this.#proc = proc;
    proc.onStderr((line) => {
      // The tail only surfaces on a non-zero exit. A container that logs a
      // fatal error and keeps running never gets there, so route each line by
      // the level the runner marked it with — at debug it is dropped outright
      // by the default `info` threshold, and `--rm` leaves no log to read.
      const { level, message } = classifyContainerLogLine(line);
      log[level](message, { container: this.name });
