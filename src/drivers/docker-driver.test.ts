...
describe('lifecycle', () => {
  ...
  // A container that logs a fatal error and keeps running never reaches the
  // non-zero-exit path above, so the tail never surfaces. Routing each line by
  // the level the runner marked it with is the only thing that reports it, and
  // `--rm` means there is no container log left to read afterwards.
  it('routes a marked container ERROR line to the host error log', async () => {
    const handle = await driver().prepare(fixtureSpec());
    await handle.start();

    cli.started.at(-1)!.proc.emitStderr('[poll-loop] ERROR Query error: boom');

    expect(log.error).toHaveBeenCalledWith(
      '[poll-loop] Query error: boom',
      expect.objectContaining({ container: 'ncl-spike-s1' }),
    );
  });

  it('leaves unmarked stderr at debug so inherited child output cannot reach the error log', async () => {
    const handle = await driver().prepare(fixtureSpec());
    await handle.start();

    cli.started.at(-1)!.proc.emitStderr('Error: connect ECONNREFUSED 127.0.0.1:443');

    expect(log.error).not.toHaveBeenCalled();
    expect(log.debug).toHaveBeenCalledWith('Error: connect ECONNREFUSED 127.0.0.1:443', expect.anything());
  });
